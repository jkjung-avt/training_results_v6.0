...
if [[ "${USE_SYNTHETIC_DATA:-0}" -eq 0 ]]; then
    ...
    TOKENIZER="${DATADIR}/${DATA_FOLDER}/tokenizer"
    PREPROC_DATA="${DATADIR}/${DATA_FOLDER}"
    _cont_mounts="${_cont_mounts},${TOKENIZER}:/workspace/llm/nemo_tokenizer:ro,${PREPROC_DATA}:/preproc_data:ro"
    mounts_to_verify="DATADIR:/preproc_data"
    if [[ -n "${LOAD_CHECKPOINTS_DIR:-}" ]]; then
        _cont_mounts="${_cont_mounts},${LOAD_CHECKPOINTS_DIR}:/checkpoints:ro"
    fi
else
